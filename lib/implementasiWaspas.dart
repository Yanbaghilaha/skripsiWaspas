...
class DecisionMakingPage extends StatefulWidget {
  ...
  @override
  _DecisionMakingPageState createState() => _DecisionMakingPageState();
}

Future<Map<String, dynamic>> fetchAlternatifData() async {
  try {
    ...
    for (QueryDocumentSnapshot<Map<String, dynamic>> doc in snapshot.docs) {
      String namaAlternatif = doc.id;
      alternatifData[namaAlternatif] = doc.data();
    }

    return alternatifData;
  } catch (error) {
    print('Error fetching alternatif data: $error');
    return {};
  }
}
...
class _DecisionMakingPageState extends State<DecisionMakingPage> {
  ...
  void initState() {
    super.initState();
    fetchKriteriaData().then((_) {
      fetchAlternatifData().then((alternatifData) {
        setState(() {
          dataAlternatif = Map.fromEntries(
            alternatifData.entries.map(
              (entry) => MapEntry(
                entry.key,
                List.generate(
                  jenisKriteria.length,
                  (index) => 0,
                ),
              ),
            ),
